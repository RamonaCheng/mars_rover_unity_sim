...
public class SCC_Particles : MonoBehaviour {
    ...
    for (int i = 0; i < exhaustEmissions.Length; i++) {
        var rateOverTime = exhaustEmissions[i].rateOverTime;
        rateOverTime.constant = Mathf.Lerp(1f, 20f, InputProcessor.inputs.throttleInput);
        exhaustEmissions[i].rateOverTime = rateOverTime;
    }
    }
